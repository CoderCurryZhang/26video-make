...
class SplitScreenTIR(Scene):
    def construct(self):
        ...
        def build_system(offset_x, n_bot, n_top, color_bot, color_top, label_bot, label_top):
            ...
            t_top = Text(label_top, font_size=20, color=WHITE).move_to(center + UP * (h_half - 0.4))
            t_bot = Text(label_bot, font_size=20, color=WHITE).move_to(center + DOWN * (h_half - 0.4))
            ...
            dynamic_grp = VGroup(inc_ray, ref_ray, tra_ray, arr_inc, arr_ref, arr_tra, src)

            return static_grp, dynamic_grp

        left_offset = -w_half / 2
        right_offset = w_half / 2

        left_static, left_dynamic = build_system(
            left_offset, n_water, n_air, col_water, col_air,
            "水 (光密介质, n=1.33)", "空气 (光疏介质, n=1.00)"
        )

        right_static, right_dynamic = build_system(
