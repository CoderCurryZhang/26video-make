...
class TotalInternalReflectionProcess(Scene):
    def construct(self):
        ...
        val_tra_pos = val_tra.get_center()
        ...
        def update_incident(mob):
            ...
            val_inc.become(
                Text(f"入射角: {theta * 180 / math.pi:.1f}°", font_size=20, color=col_inc).move_to(val_inc_pos,
                                                                                                   aligned_edge=LEFT))
        ...
        def update_refracted(mob):
            ...
            if val >= 0.9999:
                ...
                val_tra.become(
                    Text("折射角: 无 (全反射)", font_size=20, color=col_ref).move_to(val_tra_pos, aligned_edge=LEFT))
            else:
                ...
                val_tra.become(
                    Text(f"折射角: {theta_t * 180 / math.pi:.1f}°", font_size=20, color=col_tra).move_to(val_tra_pos,
                                                                                                         aligned_edge=LEFT))
